...
ft.LoadFormID=function(selector){
	var form = {};
	var self, name, v;
	$('#'+selector).each(function(i){
		self = $(this);
...
//				CKEDITOR.instances[attr.name].setData(attr.value);
			...
			ft.SetCheckboxID(attr.name, attr.value);
		}else if($('input[name="'+attr.name+'"]').is('input:radio')){
			ft.SetRadio(attr.name, attr.value);
		}else{
			self.val(attr.value);
...
/* :: Put data to input form :: */
ft.PutFormClass=function(id,data){
	var self;
	// console.log(data);
	$.each(data, function(i, attr) {
		console.log(attr);
...
//				CKEDITOR.instances[attr.name].setData(attr.value);
			...
			ft.SetCheckboxClass(attr.name, attr.value);
		}else if($('input[name="'+attr.name+'"]').is('input:radio')){
			ft.SetRadio(attr.name, attr.value);
		}else{
			self.val(attr.value);
...
ft.SetCheckboxID=function(name, value){
	if(value==1){
		$('#'+name).val(value);
		$('#'+name).iCheck('check');
	}else{
...
ft.SetCheckboxClass=function(name, value){
	if(value==1){
		$('.'+name).iCheck('check');
	}else{
		$('.'+name).iCheck('uncheck');
